...
Pages ship a strict Content-Security-Policy from `src/layouts/BaseLayout.astro`. `script-src` is `'none'` because the site sends no JavaScript; adding an Astro island or view transitions requires relaxing it to `'self'` in both the layout and `public/_headers`, which would otherwise break in the browser without failing the build. `form-action` is `'self'`: the site's feedback forms (`reportar.astro`, `OpinionHistoria.astro`) use Netlify Forms with a plain HTML POST, no JavaScript involved. See [ADR 0015](docs/adr/0015-formularios-de-feedback-con-netlify-forms.md) for why, and for why the same form still renders (with a visible notice) on the GitHub Pages build that has no Netlify backend to receive it.
